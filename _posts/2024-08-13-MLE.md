---
layout: distill
title: How to use Statistical Knowledge when analyzing your geological data?
date: 2024-08-13
description: Bridging the Gap Between Statistics and Geological Data
...
We often learn pure math and statistical concepts in class, encountering jargon like **mean**, **median**, **mode**, **standard deviation**, **variance**, and even more complex terms like **PDF** (Probability Density Function) and **CDF** (Cumulative Distribution Function). We might try hard to understand the statistics and the math behind these concepts, attempting to build a mental linkage between the math, the physical shape of the data distribution, and the statistical meaning behind it.

But when it comes to real life, when we deal with our own data—take my geological data, for example, which is the thickness of events formed by sediment gravity flow at the distal Bengal Fan—things can feel different. When I try to analyze my data, I don't necessarily have all that statistical knowledge in the forefront of my mind. Instead, I find myself literally playing with the data, trying to get to know it. I actually have some geological questions in mind before I visualize the data:
...
- **Outliers**: The super thick sediment gravity flows (SGFs) at the tail are outliers. To define outliers formally, one common method is using **mean + 2σ (standard deviations)**. In this case, for our data, this threshold is around 2 meters. Rather than calling these events "super thick," we define them as **outsized** to make the term more formal.
...
The next step is to maximize the log-likelihood function with respect to  $\mu$ and $\sigma$ to find the values that make the observed data most probable. The essence is to calculate the derivatives of the $\log_{10}(L(\mu, \sigma))$ with respect to $\mu$ and $\sigma$, and find the
...
1. **Set the derivatives to zero to find the maximum**:
...
$$
 ...
 After applying the dataset in, each thickness data point corresponds to $x_i$. $\mu$ will be obtained, which is the mean of the log-transformed data.

Same workflow for obtaining $\sigma$.
...
$$
 ...
 $\sigma$ then could be calculated after applying the dataset in, which is the standard deviation of the log-transformed data.

So far, the values of  $\mu$ and $\sigma$ that maximize the log-likelihood function are your MLE estimates. These estimates will be used to define your personalized lognormal distribution, and construct the probability density function (PDF) and cumulative distribution function (CDF) specific to your data, enabling further analysis and interpretation.

In high school, teachers just teach you the equation of calculating $\mu$ and $\sigma$, and let you memorize them for exams. Now packages in python like `pandas` can give you all the descriptive stats with just one line of code. But if you take the time to work through these calculations yourself, it really helps you understand why we’re doing them and what they actually mean, and a sense of surprise when your calculation results align with what your learned from the high school teacher. We are standing on the shoulder of the giants.

### Calculating the Cumulative Distribution Function (CDF)

Once we have the equation for the PDF, we can calculate the **Cumulative Distribution Function (CDF)** by integrating the PDF:
