...
In high school, teachers just teach you the equation of calculating $\mu$ and $\sigma$, and let you memorize them for exams. Now packages in python like `pandas` can give you all the descriptive stats with just one line of code.
```python
df.describe()
```
